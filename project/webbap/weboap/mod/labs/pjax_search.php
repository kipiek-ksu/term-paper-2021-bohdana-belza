...
/*
			...
			*/
			...
			/*$labs = get_records_select('labs', '(id='. $ch->labsid. ')','id', 'course');
			$b = '';
   			foreach ($labs as $tr) {
            	$b .= $tr->course;
            }

      		$labs = get_records_select('labs', '(course='. $b. ')','id', 'id');
			$course = '';
   			foreach ($labs as $tr) {
            	$course .= $tr->id;
            }*/ //some solution method

			$cparent = get_records_select('labs_chapters', '(parentchapter='. $ch->parentchapter. ')&&(subchapter=0)','pagenum', 'title');
            $c = '';
            foreach ($cparent as $tr) {
   			...
   			$link .= '<div class="search_object"><div class="search_object_title"><a
   				...
   				$c.' -> '.$DName .'</a></div>'; //need to fix later if create second labs
	...
	echo "текст запиту повинен містити більше ніж 2 букви";
   	...
   	//	$chapters = get_records_select('labs_chapters', 'title LIKE \'%' .$p . '%\'','pagenum', 'title, content, hidden');


    //$totalcount = count_records_sql($countsql);


    //$chapters = get_records_sql('`mdl_labs_chapters`', '(b.title LIKE '%".$text."%')OR(b.content LIKE '%".$text."%')', 'pagenum');
   // print_r($text);
