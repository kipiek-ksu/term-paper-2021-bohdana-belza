		...
		default:
			{
				...
				//FILE
					...
					if (!$newsolution->id = insert_record('vinttaskbook_solutions', $newsolution)) {
						...
						$linkyes    = 'edit.php';
						$optionsyes  = array('id'=>$cm->id, 'cid'=>$cid, 'tid'=>$tid,  'action'=>'view', 'type'=>'solution');
						$linkno     = 'edit.php';
						$optionsno = array('id'=>$cm->id, 'cid'=>$cid, 'tid'=>$tid, 'solid'=>$solid, 'action'=>'delete', 'type'=>'solution', 'confirm'=>1);
