					...
					foreach ($labtaskSolutionGrade as $onesolution)
						{
							if (($onesolution->userid==$curuserid)&&($mytask->id == $onesolution->vinttasktask)&&($onesolution->grade !='0'))
							{
								...
								$categoryTask[$size]->item[$onetask->id]->link = '<a '; 
								$categoryTask[$size]->item[$onetask->id]->link .= 'href="'.$CFG->wwwroot.'/mod/vinttaskbook/edit.php?id=35&amp;cid=';
								$categoryTask[$size]->item[$onetask->id]->link .= $onecategory->id.'&amp;tid='.$mytask->id.'&amp;solid=';
								$categoryTask[$size]->item[$onetask->id]->link .= $onesolution->id.'&amp;type=solution&amp;action=view">';
								$categoryTask[$size]->item[$onetask->id]->link .= $onesolution->grade.' [ '.$onetask->id.' | '.userdate($onesolution->timemodified).' ]</a><br>';
							}
						}
...
if ($csol)
{
            ...
            {
                echo "data2[data2.length] = new Array('".
                '<a href="'.$CFG->wwwroot.'/mod/labs/view.php?id=65">'.$headerArray[$i]->headerviewlink.'</a>'."'";
                echo ",'";
                $tt=sizeof($categoryTask[$i]->item);
                $count='0';
...
////////////////////////////////////////////////////////////////////////////////////
function get_all_labs_solutions()
{
	$labs = get_records_select('vinttaskbook_solutions', '', 'userid', 'id, vinttasktask, userid, grade');
	return $labs;
}
////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function get_labsuser($userid)
{
	$labUser = get_records_select('labs_users', 'user_id = $userid', 'id', 'id, user_id, task_id');
	return $labUser;
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

function get_lab_grades()
{
	$lab_grades = get_records_select('labs_grades', '', 'id', 'id, lab, userid, grade');
	return $lab_grades;
}
...
$quizGrades = get_quiz_grades();
		...
		//fill labs header title 
		$headerArray[sizeof($headerArray)]->headerviewlink = get_string('labs', 'grades');	
		...
		foreach($allstudents as $onestudent ) {
			...
			$select = 'user_id ='.$onestudent->id;
			$tasks = get_records_select('labs_users', $select, 'id', 'id, user_id, task_id');
			
			foreach ($tasks as $task)
			{
				for($i=3;$i<=12;$i++)
				{
					$mytask = get_record('vinttaskbook_tasks', 'vinttaskcategories', $i, 'number', $task->task_id);
					$mygrade = get_record('vinttaskbook_solutions', 'vinttasktask', $mytask->id, 'userid', $onestudent->id);
					if ($mygrade->grade != 0)
					{
						$lab_curr_Num ++;
						$lab_curr_Sum += $mygrade->grade;
					}
				}
			}
			...
			if ($lab_curr_Num != '0')
			{
				$lab_curr_Sr = $lab_curr_Sum/$lab_curr_Num;
			}
			else
			{
				$lab_curr_Sr = '0';
			}			
			//print curr_Sr lab Result
			$mcolcount=sizeof($studentRow[$mrowcount]->item);
			$studentRow[$mrowcount]->item[$mcolcount] = number_format($lab_curr_Sr, 2, ',', ' ');
			
			$lablink .= '<a href="?id='.$course->id;
			$lablink .= '&amp;action=lcats&amp;user='.$onestudent->id.'&amp;csol=true&amp;cview='.$mybook->name.'">';
			$lablink .= number_format($lab_curr_Sr, 2, ',', ' ') .'</a>';
			$studentRow[$mrowcount]->item[$mcolcount] = $lablink;
			////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
