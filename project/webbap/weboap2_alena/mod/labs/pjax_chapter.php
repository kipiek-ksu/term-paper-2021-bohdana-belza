...
if ($e) {
	if ($curparent != "")
	{
   		...
   		$chapters = get_records_select('labs_chapters', '(subchapter = 1)and(parentchapter='.$curparent.')','pagenum', 'id, pagenum, parentchapter, title, content, contenttype, element, hidden');
			...
			switch ($ch->contenttype) {
					...
					switch ($ch->element) {
						case 0:
							...
							<div class="toc_subject" style="width:205px; float:left; text-align: left; margin-left: 0px; text-indent:0px;">';
								...
								<a  onclick="gotoContent(\''.$curparent.'\',\' view.php?id='.$labs.'#subject_' . $ch->id .'\')">'. $DName .'</a>
							</div>';
							$toc .= '
							<div style="width:76px; float:right; right:0px;top:0px;">';
								if ($i != 1) {
									$toc .=  '
										<a title="'.get_string('up').'" href="move.php?id='.$labs.'&amp;chapterid='.$ch->id.'&amp;up=1&amp;sesskey='.$sesskey.'">
										<img src="'.$CFG->pixpath.'/t/up.gif" height="11" class="iconsmall" alt="'.get_string('up').'" /></a>
									';
								} //up
								if ($i != count($chapters)) {
									$toc .=  '
										<a title="'.get_string('down').'" href="move.php?id='.$labs.'&amp;chapterid='.$ch->id.'&amp;up=0&amp;sesskey='.$sesskey.'">
										<img src="'.$CFG->pixpath.'/t/down.gif" height="11" class="iconsmall" alt="'.get_string('down').'" /></a>
									';
								} //down
								$toc .=  '
									<a title="'.get_string('edit').'" href="edit.php?id='.$labs.'&amp;chapterid='.$ch->id.'">
									<img src="pix/edit.png" height="11" class="iconsmall" alt="'.get_string('edit').'" /></a>
								'; //edit
								$toc .=  '
									<a title="'.get_string('delete').'" href="delete.php?id='.$labs.'&amp;chapterid='.$ch->id.'&amp;sesskey='.$sesskey.'">
									<img src="pix/del.png" height="11" class="iconsmall" alt="'.get_string('delete').'" /></a>
								'; //delete
								$toc .= '
									<a title="'.get_string('addafter', 'labs').'" href="edit.php?id='.$labs.'&amp;pagenum='.$ch->pagenum.'&amp;parentchapter='.$ch->parentchapter.'">
									<img src="pix/add.png" height="11" class="iconsmall" alt="'.get_string('addafter', 'labs').'" /></a>
								';  //add
							$toc .= '
							</div></div>';
							$toc .='
						...
						case 1:
							...
							<div class="toc_fragment" style="width:190px; float:left; left:50px;">';
								$toc .= '<a onclick="gotoContent(\''.$curparent.'\',\' view.php?id='.$labs.'#fragment_' . $ch->id.'\')">'. $title .'</a>
							</div>';
							$toc .='
							<div style="width:76px; float:right; right:0px;top:0px;">';
								if ($i != 1) {
						...
						case 2:
							...
							<div><div class="toc_example" style="width:190px; float:left; left:50px;">';
								$toc .= '<a onclick="gotoContent(\''.$curparent.'\',\' view.php?id='.$labs.'#example_' . $ch->id.'\')">'. $title .'</a>
							</div>';
							$toc .='
							<div style="width:76px; float:right; right:0px;top:0px;">';
								if ($i != 1) {
						...
						case 3:
							...
							<div><div class="toc_problem" style="width:190px; float:left; left:50px;">';
								$toc .= '<a onclick="gotoContent(\''.$curparent.'\',\' view.php?id='.$labs.'#problem_' . $ch->id.'\')">'. $title .'</a>
							</div>';
							$toc .='
							<div style="width:76px; float:right; right:0px;top:0px;">';
								if ($i != 1) {
						...
						case 4:
							...
							<div><div class="toc_comment" style="width:190px; float:left; left:50px;">';
								$toc .= '<a onclick="gotoContent(\''.$curparent.'\',\' view.php?id='.$labs.'#comment_' . $ch->id.'\')">'. $title .'</a>
							</div>';
							$toc .='
							<div style="width:76px; float:right; right:0px;top:0px;">';
								if ($i != 1) {
				...
				case 1: //first level
					if ($subtempid != $ch->id) {
						...
						$toc .= '<a onclick="gotoContent(\''.$curparent.'\',\' view.php?id='.$labs.'#stext_' . $ch->id.'\')">'. $title .'</a>';
				...
				case 3:
					...
					$toc .= '<a onclick="gotoContent(\''.$curparent.'\',\' view.php?id='.$labs.'#listing_' . $ch->id.'\')">'. $title .'</a>';
					$toc .='
	...
	if ($curparent != "") {
		$chapters = get_records_select('labs_chapters', '(subchapter = 1)and(element=0)and(parentchapter='.$curparent.')','pagenum', 'id, title, content, hidden');
		foreach ($chapters as $ch) {
			if ($ch->title=='') {
				$DName ='******';
			}
			...
			$toc .=  '<a  onclick="gotoContent(\''.$curparent.'\',\' view.php?id='.$labs.'#subject_' . $ch->id .'\')">'. $DName .'</a>';
			$toc .= '</div>';
		}
	}
	echo $toc;
