<?PHP // $Id: generateimscp.php,v 1.1.2.3 2007/05/20 06:02:00 skodak Exp $
...
$strtop  = get_string('top', 'labs');

add_to_log($course->id, 'labs', 'generateimscp', 'generateimscp.php?id='.$cm->id, $labs->id, $cm->id);

/// Get all the chapters
    $chapters = get_records('labs_chapters', 'labsid', $labs->id, 'pagenum');

/// Generate the manifest and all the contents
    chapters2imsmanifest($chapters, $labs, $cm);

/// Now zip everything
    make_upload_directory('temp');
    $zipfile = $CFG->dataroot . "/temp/". time() . '.zip';
    $files = get_directory_list($CFG->dataroot . "/$cm->course/moddata/labs/$labs->id", basename($zipfile), false, true, true);
    foreach ($files as $key => $value) {
        $files[$key] = $CFG->dataroot . "/$cm->course/moddata/labs/$labs->id/" . $value;
    }
    zip_files($files, $zipfile);
/// Now delete all the temp dirs
    delete_dir_contents($CFG->dataroot . "/$cm->course/moddata/labs/$labs->id");
/// Now serve the file
    send_file($zipfile, clean_filename($labs->name) . '.zip', 86400, 0, false, true);

/**
 * This function will create the default imsmanifest plus contents for the labs chapters passed as array
 * Everything will be created under the labs moddata file area *
 */
function chapters2imsmanifest ($chapters, $labs, $cm) {
...
/// Moodle and labs version
    $moodle_release = $CFG->release;
    $moodle_version = $CFG->version;
    $labs_version   = get_field('modules', 'version', 'name', 'labs');

/// Load manifest header
    $imsmanifest .= '<?xml version="1.0" encoding="UTF-8"?>
<!-- This package has been created with Moodle ' . $moodle_release . ' (' . $moodle_version . '), labs module version ' . $labs_version . ' - http://moodle.org -->
<!-- One idea and implementation by Eloy Lafuente (stronk7) and Antonio Vicent (C) 2001-3001 -->
<manifest xmlns="http://www.imsglobal.org/xsd/imscp_v1p1" xmlns:imsmd="http://www.imsglobal.org/xsd/imsmd_v1p2" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" identifier="MANIFEST-' . md5($CFG->wwwroot . '-' . $labs->course . '-' . $labs->id) . '" xsi:schemaLocation="http://www.imsglobal.org/xsd/imscp_v1p1 imscp_v1p1.xsd http://www.imsglobal.org/xsd/imsmd_v1p2 imsmd_v1p2p2.xsd">
  <organizations default="MOODLE-' . $labs->course . '-' . $labs->id . '">
    <organization identifier="MOODLE-' . $labs->course . '-' . $labs->id . '" structure="hierarchical">
      <title>' . htmlspecialchars($labs->name) . '</title>';

/// Create the temp directory
    $moddir = "$cm->course/moddata/labs/$labs->id";
    make_upload_directory($moddir);

/// For each chapter, create the corresponding directory and save contents there

/// To store the prev level (labs only have 0 and 1)
    $prevlevel = null;
    foreach ($chapters as $chapter) {
    /// Calculate current level ((labs only have 0 and 1)
    ...
    /// Create the static html file + local attachments (images...)
        ...
        $chaptercontent = chapter2html($chapter, $labs->course, $labs->id);
        file_put_contents($CFG->dataroot . "/" . $chapterdir . "/index.html", $chaptercontent->content);
    /// Add the imsitems
        $imsitems .= $currspaces .'        <item identifier="ITEM-' . $labs->course . '-' . $labs->id . '-' . $chapter->pagenum .'" isvisible="true" identifierref="RES-' . $labs->course . '-' . $labs->id . '-' . $chapter->pagenum . '">
 ' . $currspaces . '         <title>' . htmlspecialchars($chapter->title) . '</title>' . "\n";
    ...
    /// Now add the dependency to css
        $cssdependency = "\n" . '      <dependency identifierref="RES-' . $labs->course . '-'  . $labs->id . '-css" />';
    /// Now build the resources section
        $imsresources .= '    <resource identifier="RES-' . $labs->course . '-'  . $labs->id . '-' . $chapter->pagenum . '" type="webcontent" xml:base="' . $chapter->pagenum . '/" href="index.html">
...
$cssresource = '    <resource identifier="RES-' . $labs->course . '-'  . $labs->id . '-css" type="webcontent" xml:base="css/" href="styles.css">
...
/// Now send the css resource
    ...
    file_put_contents($CFG->dataroot . "/" . $moddir . "/css/styles.css", file_get_contents("$CFG->dirroot/mod/labs/generateimscp.css"));
}
 ...
 */
function chapter2html($chapter, $courseid, $labsid) {
    ...
    /// Build the array of local files
        ...
        /// Copy files to current chapter directory
            if (file_exists($CFG->dataroot . '/' . $courseid . '/' . $value)) {
                copy($CFG->dataroot . '/' . $courseid . '/' . $value, $CFG->dataroot . '/' . $courseid . '/moddata/labs/' . $labsid . '/' . $chapter->pagenum . '/' . basename ($value));
            }
        }
