<?php //$Id: edit_form.php,v 1.1.2.7 2007/05/15 18:23:44 skodak Exp $
...
class group_edit_form extends moodleform {
    ...
    function definition () {
		...
		$t = get_records_sql("SELECT a.id, a.firstname, a.lastname FROM {$CFG->prefix}user a INNER JOIN {$CFG->prefix}role_assignments b ON a.id = b.userid WHERE b.roleid = 3 ORDER BY lastname, firstname");
		$teachers = array('-1' => '');
		foreach ($t as $teacher)
			$teachers[$teacher->id] = $teacher->lastname . " " . $teacher->firstname;
        $mform->addElement('select', 'ownerid', 'Owner', $teachers);
