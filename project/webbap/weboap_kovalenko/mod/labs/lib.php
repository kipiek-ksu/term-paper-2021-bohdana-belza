...
if (!isset($CFG->labs_tocwidth)) {
    set_config("labs_tocwidth", 180);  // default toc width
}

function labs_get_numbering_types() {
    return array (NUM_NONE       => get_string('numbering0', 'labs'),
                  NUM_NUMBERS    => get_string('numbering1', 'labs'),
                  NUM_BULLETS    => get_string('numbering2', 'labs'),
                  NUM_INDENTED   => get_string('numbering3', 'labs') );
}

/// Library of functions and constants for module 'labs'

function labs_add_instance($labs) {
/// Given an object containing all the necessary data,
/// (defined by the form in mod.html) this function
/// will create a new instance and return the id number
/// of the new instance.

    $labs->timecreated = time();
    $labs->timemodified = $labs->timecreated;
    if (!isset($labs->customtitles)) {
        $labs->customtitles = 0;
    }
    if (!isset($labs->disableprinting)) {
        $labs->disableprinting = 0;
    }

    return insert_record('labs', $labs);
}


function labs_update_instance($labs) {
/// Given an object containing all the necessary data,
/// (defined by the form in mod.html) this function
/// will update an existing instance with new data.

    $labs->timemodified = time();
    $labs->id = $labs->instance;
    if (!isset($labs->customtitles)) {
        $labs->customtitles = 0;
    }
    if (!isset($labs->disableprinting)) {
    ...
    return update_record('labs', $labs);
}


function labs_delete_instance($id) {
/// Given an ID of an instance of this module,
/// this function will permanently delete the instance
/// and any data that depends on it.

    if (! $labs = get_record('labs', 'id', $id)) {
        return false;
    }

    $result = true;

    delete_records('labs_chapters', 'labsid', $labs->id);

    if (! delete_records('labs', 'id', $labs->id)) {
        $result = false;
    }
...
function labs_get_types() {
    ...
    $type->type = 'labs';
    $type->typestr = get_string('modulename', 'labs');
    $types[] = $type;

    return $types;
}
function labs_user_outline($course, $user, $mod, $labs) {
...
function labs_user_complete($course, $user, $mod, $labs) {
...
function labs_print_recent_activity($course, $isteacher, $timestart) {
/// Given a course and a time, this module should find recent activity
/// that has occurred in labs activities and print it out.
...
function labs_get_participants($labsid) {
//Must return an array of user records (all data) who are participants
//for a given instance of labs. Must include every user involved
...
function labs_scale_used ($labsid,$scaleid) {
//This function returns if a scale is being used by one labs
...
//as reference.
    ...
    //$rec = get_record('labs','id',$labsid,'scale',"-$scaleid");
    //
...
//////////////////////////////////////////////////////////////////////////////////////
/// Any other labs functions go here.  Each of them must have a name that
/// starts with labs_

//check chapter ordering and
//make sure subchapter is not first in labs
//hidden chapter must have all subchapters hidden too
function labs_check_structure($labsid) {
    if ($chapters = get_records('labs_chapters', 'labsid', $labsid, 'pagenum', 'id, pagenum, subchapter, hidden')) {
        $first = true;
        $hidesub = true;
        $i = 1;
...
function labs_edit_button($id, $courseid, $chapterid) {
    global $CFG, $USER;
    ...
    if (isteacheredit($courseid)) {
        ...
        return '<form method="get" action="'.$CFG->wwwroot.'/mod/labs/view.php"><div>'.
...
function labs_log($str1, $str2, $level = 0) {
    switch ($level) {
        case 1:
...
function labs_prepare_link($ref) {
    if ($ref == '') {
        return '';
    }
...
function labs_read_chapter($base, $ref) {
    $file = $base.'/'.$ref;
    if (filesize($file) <= 0 or !is_readable($file)) {
        labs_log($ref, get_string('error'), 2);
        return;
    }
    //first read data
...
function labs_relink($id, $labsid, $courseid) {
    global $CFG;
    ...
    $chapters = get_records('labs_chapters', 'labsid', $labsid, 'pagenum', 'id, pagenum, title, content, importsrc');
    $originals = array();
    foreach($chapters as $ch) {
        ...
        if ($ch->importsrc && preg_match_all('/(<img[^>]+src=")([^"]+)("[^>]*>)/i', $ch->content, $images)) {
            for($i = 0; $i<count($images[0]); $i++) {
                if (!preg_match('/[a-z]+:/i', $images[2][$i])) { // not absolute link
                    $link = labs_prepare_link($base.$images[2][$i]);
                    if ($link == '') {
                        continue;
                    }
                    $origtag = $images[0][$i];
                    ...
                    $modified = true;
                    labs_log($ch->title, $images[2][$i].' --> '.$link);
                }
            }
        }
        //css relinking (dlnsk)
        if ($ch->importsrc && preg_match_all('/(<link[^>]+href=")([^"]+)("[^>]*>)/i', $ch->content, $csslinks)) {
            for($i = 0; $i<count($csslinks[0]); $i++) {
                if (!preg_match('/[a-z]+:/i', $csslinks[2][$i])) { // not absolute link
                    $link = labs_prepare_link($base.$csslinks[2][$i]);
                    if ($link == '') {
                        continue;
                    }
                    ...
                    $ch->content = str_replace($origtag, $newtag, $ch->content);
                    $modified = true;
                    labs_log($ch->title, $csslinks[2][$i].' --> '.$link);
                }
            }
        }
        ...
        if ($ch->importsrc && preg_match_all('/(<embed[^>]+src=")([^"]+)("[^>]*>)/i', $ch->content, $embeds)) {
            for($i = 0; $i<count($embeds[0]); $i++) {
                if (!preg_match('/[a-z]+:/i', $embeds[2][$i])) { // not absolute link
                    $link = labs_prepare_link($base.$embeds[2][$i]);
                    if ($link == '') {
                        continue;
                    }
                    $origtag = $embeds[0][$i];
                    ...
                    $ch->content = str_replace($origtag, $newtag, $ch->content);
                    $modified = true;
                    labs_log($ch->title, $embeds[2][$i].' --> '.$link);
                }
            }
        }
            ...
            for($i = 0; $i<count($params[0]); $i++) {
                if (preg_match('/(value=\s*")([^"]+)(")/i', $params[0][$i], $values)) {
                    if (!preg_match('/[a-z]+:/i', $values[2])) { // not absolute link
                        $link = labs_prepare_link($base.$values[2]);
                        if ($link == '') {
                            continue;
                        }
                        ...
                        $modified = true;
                        labs_log($ch->title, $values[2].' --> '.$link);
                    }
                }
            }
        }
        ...
        if ($ch->importsrc && preg_match_all('/(<applet[^>]+codebase=")([^"]+)("[^>]*>)/i', $ch->content, $codebases)) {
            for($i = 0; $i<count($codebases[0]); $i++) {
                if (!preg_match('/[a-z]+:/i', $codebases[2][$i])) { // not absolute link
                    $link = labs_prepare_link($base.$codebases[2][$i]);
                    if ($link == '') {
                        continue;
                    }
                    ...
                    $ch->content = str_replace($origtag, $newtag, $ch->content);
                    $modified = true;
                    labs_log($ch->title, $codebases[2][$i].' --> '.$link);
                }
            }
        }
        ...
        if ($ch->importsrc && preg_match_all('/(<a[^>]+href=")([^"^#]*)(#[^"]*)?("[^>]*>)/i', $ch->content, $links)) {
            for($i = 0; $i<count($links[0]); $i++) {
                ...
                && !preg_match('/[a-z]+:/i', $links[2][$i])) { //not absolute link
                    $origtag = $links[0][$i];
                    $target = labs_prepare_link($rel.$links[2][$i]); //target chapter
                    if ($target != '' && array_key_exists($target, $originals)) {
                        $o = $originals[$target];
                        $newtag = $links[1][$i].$CFG->wwwroot.'/mod/labs/view.php?id='.$id.'&chapterid='.$o->id.$links[3][$i].$links[4][$i];
                        $newtag = preg_replace('/target=[^\s>]/i','', $newtag);
                        $ch->content = str_replace($origtag, $newtag, $ch->content);
                        $modified = true;
                        labs_log($ch->title, $links[2][$i].$links[3][$i].' --> '.$CFG->wwwroot.'/mod/labs/view.php?id='.$id.'&chapterid='.$o->id.$links[3][$i]);
                    } else if ($target!='' && (!preg_match('/\.html$|\.htm$/i', $links[2][$i]))) { // other relative non html links converted to download links
                        $target = labs_prepare_link($base.$links[2][$i]);
                        $origtag = $links[0][$i];
                        $newtag = $links[1][$i].$target.$links[4][$i];
                        $ch->content = str_replace($origtag, $newtag, $ch->content);
                        $modified = true;
                        labs_log($ch->title, $links[2][$i].' --> '.$target);
                    }
                }
            }
        }
