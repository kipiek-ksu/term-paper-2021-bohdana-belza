...
/// read chapters
$select = $allowedit ? "labsid = $labs->id" : "labsid = $labs->id AND hidden = 0";
...
if (!$chapters) {
    if ($allowedit) {
        ...
        error('Error reading labs chapters.');
    }
}
...
/// chapter not part of this labs!
if ($chapter->labsid != $labs->id) {
...
$menu = '
	<ul id="nav">
		<li class="page_item">
			...
			<a href="index.php?id='.$course->id.'">'.$strlabss.'</a> -> '.$labs->name.'</li>
	</ul>';
print_header( '&nbsp;',
              '&nbsp;',
              /*"$navigation <a href=\"index.php?id=$course->id\">$strlabss</a> -> $labs->name"*/$menu,
              '','
...
#virtlabs {	display:block; width: 1000px; height: 530px; left:0px; top:0px; float:top; border:0px solid #ffddee;}
...
"function TransferFromListing(parent) {\n".
	...
	"prevPageContent=".
		"'<html><head><script '+'type=\"'+'text/'+'javascript\" src=\"".$CFG->wwwroot."/mod/labs/gits3.'+'js\">'+'</'+'script></head><body onresize=\"ScreenResize()\">'+\n".
