					...
					if ($onetask->vinttaskcategories==$onecategory->id)
					{
						foreach ($vinttaskSolutionGrade as $onesolution)
						{
							if (($onesolution->userid==$curuserid)&&($onetask->id == $onesolution->vinttasktask)&&($onesolution->grade !='0'))
							{
								...
								$categoryTask[$size]->item[$onetask->id]->link = '<a href="'.$CFG->wwwroot.'/mod/vinttaskbook/edit.php?id=35&amp;cid=';
...
if ($csol)
{
			...
			if ($totalCount!='0')
            {
                echo "'".number_format($totalSum/$totalCount, 2, ',', ' ')."'";
            } else
            {
...
function get_all_students($view_by_student)
{
	...
	else
	{
		...
		if (isset($group_id)) { //Group(s) is set
			$sql_str = "";
			if (in_array(-1, $group_id)) {
				if (isset($teacher_id) and $teacher_id != -1) {
					//All groups of specific Teacher
					$students = get_records_sql("SELECT d.id, d.firstname, d.lastname FROM (SELECT b.userid FROM mdl_groups a INNER JOIN mdl_groups_members b ON a.id = b.groupid WHERE a.ownerid = {$teacher_id}) c INNER JOIN mdl_user d ON c.userid = d.id ORDER BY firstname");
				} else {
					//All groups of all Teachers
					$students = get_records_sql("SELECT id, firstname, lastname FROM mdl_user ORDER BY firstname");
				}
			} else {
				if (isset($teacher_id) and $teacher_id != -1) {
					//Specified groups of specified Teacher
					$sql_str = " and (a.id = ";
					$sql_str .= implode(" or a.id = ", $group_id);
					$sql_str .= ")";
					$students = get_records_sql("SELECT d.id, d.firstname, d.lastname FROM (SELECT b.userid FROM mdl_groups a INNER JOIN mdl_groups_members b ON a.id = b.groupid WHERE a.ownerid = {$teacher_id}{$sql_str}) c INNER JOIN mdl_user d ON c.userid = d.id ORDER BY firstname");
					if (!$students) {
						$students = get_records_sql("SELECT d.id, d.firstname, d.lastname FROM (SELECT b.userid FROM mdl_groups a INNER JOIN mdl_groups_members b ON a.id = b.groupid WHERE a.ownerid = {$teacher_id}) c INNER JOIN mdl_user d ON c.userid = d.id ORDER BY firstname");
					}
				} else {
					//Specified groups of all Teachers
					$sql_str = "a.id = ";
					$sql_str .= implode(" or a.id = ", $group_id);
					$students = get_records_sql("SELECT d.id, d.firstname, d.lastname FROM (SELECT b.userid FROM mdl_groups a INNER JOIN mdl_groups_members b ON a.id = b.groupid WHERE {$sql_str}) c INNER JOIN mdl_user d ON c.userid = d.id ORDER BY firstname");
				}
			}
		} else {
			if (isset($teacher_id) and $teacher_id != -1) {
...
{ 
    $solutions = get_records_select('vinttaskbook_solutions', '', 'userid', 'id, userid, grade, vinttasktask');
	return $solutions;
}
...
function grade_view_labs($user) {
	$headerArray=null;
	$headerArray[sizeof($headerArray)]->headerviewlink = "Работа";
	$headerArray[sizeof($headerArray)]->headerviewlink = "Завд. №1";
	$headerArray[sizeof($headerArray)]->headerviewlink = "Завд. №2";
	$headerArray[sizeof($headerArray)]->headerviewlink = "Завд. №3";
	$headerArray[sizeof($headerArray)]->headerviewlink = "Завд. №4";
	$headerArray[sizeof($headerArray)]->headerviewlink = "";
	$studentRow=null;
	$mrowcount = 0;
	$labs = get_records_sql("SELECT id, name FROM mdl_lab_categories ORDER BY id, name");
	$sum_labs = 0;
	for ($i = 1; $i <= sizeof($labs); $i++) {
		$cid = get_record_sql("SELECT id FROM mdl_vinttaskbook_categories WHERE number = $i")->id;
		$tasks = get_records_sql("SELECT b.id FROM ( SELECT id, task_id FROM mdl_labs_users WHERE user_id = $user ) a INNER JOIN mdl_vinttaskbook_tasks b ON a.task_id = b.number WHERE b.vinttaskcategories = $cid ORDER BY a.id");
		$grades = get_records_sql("SELECT d.id as taskid, c.id, c.grade FROM mdl_vinttaskbook_solutions c INNER JOIN ( SELECT b.id FROM ( SELECT id, task_id FROM mdl_labs_users WHERE user_id = $user ) a INNER JOIN mdl_vinttaskbook_tasks b ON a.task_id = b.number WHERE b.vinttaskcategories = $cid ORDER BY a.id ) d ON c.vinttasktask = d.id WHERE userid = $user");
		$g = array(0, 0, 0, 0);
		$k = 0;
		$sum = 0;
		foreach ($tasks as $task) {
			if (isset($grades[$task->id])) {
				$g[$k] = $grades[$task->id]->grade;
				$sum += $g[$k];
				$k++;
			}
		}
		...
		$mrowcount=sizeof($studentRow);
		$mcolcount=sizeof($studentRow[$mrowcount]->item);
		$studentRow[$mrowcount]->item[$mcolcount] = $labs[$i]->name;
		$mcolcount=sizeof($studentRow[$mrowcount]->item);
		$studentRow[$mrowcount]->item[$mcolcount] = number_format($g[0], 2, ',', ' ');
		$mcolcount=sizeof($studentRow[$mrowcount]->item);
		$studentRow[$mrowcount]->item[$mcolcount] = number_format($g[1], 2, ',', ' ');
		$mcolcount=sizeof($studentRow[$mrowcount]->item);
		$studentRow[$mrowcount]->item[$mcolcount] = number_format($g[2], 2, ',', ' ');
		$mcolcount=sizeof($studentRow[$mrowcount]->item);
		$studentRow[$mrowcount]->item[$mcolcount] = number_format($g[3], 2, ',', ' ');
		$mcolcount=sizeof($studentRow[$mrowcount]->item);
		$studentRow[$mrowcount]->item[$mcolcount] = number_format($sum / 4, 2, ',', ' ');
	}
	//Вывод итога
	$headerArray[sizeof($headerArray) - 1]->headerviewlink = "Підсумок (" . number_format($sum_labs / sizeof($labs), 2, ',', ' ') . ")";
	$length=sizeof($headerArray);
	echo "<script>
	function headerInit() {
		return new Array(";
	...
	{
		$ttemp='';
		$ttemp = 'data[data.length] = new Array(';
		foreach ($oneStudentRow->item as $oneItem)
		{
			$ttemp .= '\''.$oneItem.'\',';
		}
		$ttemp = substr($ttemp, 0, $ttemp.length - 1);
		$ttemp .= ');';
		echo $ttemp."\n";
	}
...
// displays all grades for the course
	...
	$vinttaskbook = get_record("vinttaskbook", "course", $course->id);
		...
		//labs
		$headerArray[sizeof($headerArray)]->headerviewlink = "Лабораторні";
        ...
        $studentRow=null;
		$labs = get_records_sql("SELECT id, name FROM mdl_lab_categories ORDER BY id, name");
		$cids = get_records_sql("SELECT number, id FROM mdl_vinttaskbook_categories");
		$alltasks = get_records_sql("SELECT id, vinttaskcategories, number FROM mdl_vinttaskbook_tasks");
		$alltasknumbers = get_records_sql("SELECT * FROM mdl_labs_users");
		foreach($allstudents as $onestudent ) {
            $mrowcount=sizeof($studentRow);
            $mcolcount=sizeof($studentRow[$mrowcount]->item);
			...
			//ЛАБЫ!!! Ужасно но работает. Ужасно потому что каждый раз пересчитываются ср. арифм. по всем лабам студентов
			//В дальнейшем можно хранить данные об лабах в таблице mdl_labs_grades
			$sum = 0;
			$mytasknums = array();
			$n = 0;
			foreach ($alltasknumbers as $alltasknumber) {
				if ($alltasknumber->user_id == $onestudent->id) {
					$mytasknums[$n++] = $alltasknumber->task_id;
				}
			}
			for ($i = 1; $i <= sizeof($labs); $i++) {
				$mytasknumbers = array();
				...
				foreach($alltasks as $alltask) {
					if ($alltask->vinttaskcategories == $cid AND in_array($alltask->number, $mytasknums)) {
						$mytasknumbers[$n++] = $alltask->id;
					}
				}
				foreach ($vinttaskSolutionGrade as $solution) {
					if (in_array($solution->vinttasktask, $mytasknumbers) AND $solution->userid == $onestudent->id) {
						$sum += $solution->grade;
					}
				}
			}
			$labsAVG = $sum / sizeof($labs) / 4;
			
			$mcolcount=sizeof($studentRow[$mrowcount]->item);
			$studentRow[$mrowcount]->item[$mcolcount] = "<a href=?action=labs&user=$onestudent->id&id=$course->id>" . number_format($labsAVG, 2, ',', ' ') . "</a>";
			...
			$totalNum ='4';
			$totalSum = $quiz_curr_Sr+$quiz_fin;
			if ($totalNum != '0')
			{
				$totalSr = ($totalSum+$taskSr+$labsAVG)/($totalNum);
			}
			else
			{
				$totalSr = '0';
