...
//		$sql = 'select count(distinct userid) from '.$CFG->prefix.'user_teachers';
	...
	function get_content() {
		...
		if ( $CFG->block_ministats_show_acrive_users && isset($stats['activeUserCount']) )
			$this->content->text .= get_string('activeusers').': '.$stats['activeUserCount'].' ('.get_string('last_days', 'block_ministats', $CFG->block_ministats_active_users_days).')<br />';
		
		if ( $CFG->block_ministats_show_teachers && isset($stats['teacherCount']) )
			$this->content->text .= $COURSE->teachers.': '.$stats['teacherCount'].'<br />';


		$this->content->footer = get_string('updated_on', 'block_ministats').'<br>&nbsp;'.userdate( $stats['timestamp'] );
