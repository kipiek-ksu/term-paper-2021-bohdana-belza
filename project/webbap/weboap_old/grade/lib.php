<?php // $Id: lib.php,v 1.65.2.6 2007/08/01 05:10:54 moodler Exp $
...
function grade_view_category_grades($view_by_student) {
    ...
    if ($csol)
    {
			...
			//echo "firstname=".$currstudent->firstname." lastname=".$currstudent->lastname;
			
            //one chapter for current course-id from vinttaskbook 
            $vinttaskbook = get_record("vinttaskbook", "course", $course->id);      
         
            //get categories for current vinttaskbook-id from vinttaskbook_categories
            $select = "vinttaskbook =". $vinttaskbook->id; 
            $vinttaskcategories = get_records_select('vinttaskbook_categories', $select, 'id', 'id, number, name');
            //$vinttaskcategories = get_records("vinttaskbook_categories", "vinttaskbook", $vinttaskbook->id, "number");
            
            //get tasks for onecategory-id from vinttaskbook_tasks
            $select = "vinttaskbook =". $vinttaskbook->id; 
            $vinttasktasks = get_records_select('vinttaskbook_tasks', $select, 'id', 'id, number, name, vinttaskcategories');
            //$vinttasktasks = get_records("vinttaskbook_tasks", "vinttaskcategories", $vinttaskbook->id, "number");
            
            
            //all solutions from vinttaskbook
            $select = "vinttaskbook = 1";//hardcode greatings to vinttaskbook authors :( 
            $vinttaskSolutionGrade = get_records_select('vinttaskbook_solutions ', $select, 'userid', 'id, userid, vinttasktask, grade, timecreated, timemodified');

			//bad choise :( , but only work for 100%
			//foreach ($vinttaskcategories as $onecategory)
		    //{
			//	foreach ($vinttasktasks as $onetask)
			//	{	
			//		if ($onetask->vinttaskcategories==$onecategory->id)
			//		{
			//			foreach ($vinttaskSolutionGrade as $onesolution)
			//			{
			//				if (($onesolution->userid==$curuserid)&&($onetask->id == $onesolution->vinttasktask)&&($onesolution->grade !='0'))
			//				{
			//					echo 'onecategory='.$onecategory->id.' onetask='.$onetask->id.' onesolutionGRADE='.$onesolution->grade.'<br>';
			//				}
			//			}
			//		}
			//	}
	...
	//$taskid='32';
	//$vinttasktask = get_record("vinttaskbook_tasks", "id", $taskid);
	...
	//$select = "vinttaskbook = 1";//$allowedit ? "bookid = $book->id" : "bookid = $book->id AND hidden = 0";
	//echo $select;
	//$vinttaskbook = get_records_select('vinttaskbook_solutions', $select, 'vinttasktask', 'id, vinttasktask, userid, grade, language, timecreated, timemodified');

	//echo 'fffffff'. $vinttaskbook.'eeeee';
	//foreach($vinttaskbook as $onebook) {
	//            echo $onebook->solution;
	//}  
	...
	//}
            ...
            //if ($csol)
            //{
				//fill solution header title
			//	foreach ($vinttaskcategories as $onecategory)
			//	{
			//		$headerArray[sizeof($headerArray)]->headerviewlink = $onecategory->name;
			//	}
            //}

            if ($csol)
            {
                        //$mrowcount=sizeof($studentRow);
                        //$mcolcount=sizeof($studentRow[$mrowcount]->item);
                        ...
                        //$sccount=sizeof($studentcategory);
            
                        //if (has_capability('moodle/course:viewcoursegrades', $context)) {
                        //    if ($view_by_student != -1) {
                        //        $studentviewlink = '<a href="'.$CFG->wwwroot.'/user/view.php?id='.$curuserid.'&amp;course='.$course->id.'">';
                        //    }
                        //    else {
                        //        $studentviewlink = '<a href="?id='.$course->id.'&amp;action=vcats&amp;user='.$curuserid.'&amp;cview='.$cview.'">';
                        //    }
                        //    $studentviewlink .= $grades_by_student[$curuserid]['student_data']['firstname'].' '.$grades_by_student[$curuserid]['student_data']['lastname'].'</a>';
                            //$row .= '<th class="fullname" scope="row">'.$student_link.'</th>';
                        //     $studentcategory->studviewlink = $studentviewlink;
                        //}
                        //echo "student".$curuserid."link=".$studentviewlink."<br>";
						

                   //     foreach ($vinttaskcategories as $onecategory)
                   //     {
                            //echo "category=".$onecategory->name.":<br>";
                            //$headerArray[sizeof($headerArray)]->headerviewlink = '<a href="'.$CFG->wwwroot.'/mod/vinttaskbook/view.php?id=35&cid='.$onecategory->id.'">'.$onecategory->name.'</a>';
				   //		$size=sizeof($headerArray);
				   //		$headerArray[$size]->headerviewlink = $onecategory->name;
				   //		$headerArray[$size]->id = $onecategory->id;
                            //$categoryTask[sizeof($headerArray)];
                   //         foreach ($vinttasktasks as $onetask)
                   //         {                                
                   //             if ($onetask->vinttaskcategories==$onecategory->id)
                   //             {
                                    //$categoryTask[sizeof($headerArray)]->item[sizeof($categoryTask[sizeof($headerArray)]->item)]='0';
                                    //echo "itemsize".sizeof($categoryTask[sizeof($headerArray)]->item);
                                    //echo "number=".$onetask->number." task=".$onetask->name."<br>";
                   //             }
                   //         }
                   //     }

/*                        $searchsql = "
                            SELECT t1.userid, t1.grade, t1.language, t1.id, t1.timecreated, t1.timemodified, t1.vinttasktask, t2.vinttaskcategories
                            FROM `mdl_vinttaskbook_solutions` AS t1
                            INNER JOIN `mdl_vinttaskbook_tasks` AS t2
                            INNER JOIN `mdl_vinttaskbook_categories` AS t3
                            ON (t2.vinttaskcategories=t3.id) AND (t1.vinttasktask=t2.id)  ORDER BY t1.vinttasktask
                        ";
                        //$searchsql = "
                        //    SELECT t1.userid, t1.grade, t1.language, t1.id, t1.timecreated, t1.timemodified, t1.vinttasktask, t2.vinttaskcategories
                        //    FROM `mdl_vinttaskbook_solutions` t1
                        //    INNER JOIN `mdl_vinttaskbook_tasks` t2
                        //    INNER JOIN `mdl_vinttaskbook_categories` t3
                        //    ON (t2.vinttaskcategories=t3.id) AND (t1.vinttasktask=t2.id)
                        //";
                        
                        //AND (t3.vinttaskbook='".$vinttaskbook->id."'))AND (t1.vinttaskbook='1')AND (t1.userid='3')
                        $vinttaskSG = get_records_sql($searchsql);                       
                        //$vinttaskSG = get_recordset_sql($searchsql);

						foreach ($vinttaskSG as $onesolution)
                        {
                            if ($onesolution->userid==$curuserid)
                            {
                                if ($onesolution->grade>'0')
                                {
                                    $categoryTask[$onesolution->vinttaskcategories]->item[$onesolution->vinttasktask] =$onesolution->grade;
                                    //echo "grade=".$onesolution->grade." task=".$onesolution->vinttasktask." category=".$onesolution->vinttaskcategories." userid=".$onesolution->userid."<br>";
                                    //echo "grade=".$categoryTask[$onesolution->vinttaskcategories]->item[$onesolution->vinttasktask];
                                }
                            }                         
                        }
*/
                        //echo "ctr=".$ctrowcount."ctc=".$ctcolcount;
                        
                        //foreach ($categoryTask as $onecategory)
                        //{
                        //    echo "<br>";
                        //    foreach ($onecategory->item as $onetask)
                        //    {
                        //        echo "onecat=".$onetask;
                        //    }
                        //}
			...
			foreach ($vinttaskcategories as $onecategory)
		    {
				...
				$headerArray[$size]->headerviewlink = $onecategory->name;
				$headerArray[$size]->id = $onecategory->id;
				//echo "cat= ". $onecategory->name." id= ".  $onecategory->id. "<br>";
				foreach ($vinttasktasks as $onetask)
				{	
					if ($onetask->vinttaskcategories==$onecategory->id)
					{
						foreach ($vinttaskSolutionGrade as $onesolution)
						{
							if (($onesolution->userid==$curuserid)&&($onetask->id == $onesolution->vinttasktask)&&($onesolution->grade !='0'))
							{
								...
								//$categoryTask[$size]->item[$onetask->id]->link .= 'onMouseOver="docTips.show('."'welcome'".')"onMouseOut="docTips.hide()" ';
								...
								$categoryTask[$size]->item[$onetask->id]->link .= $onesolution->grade.' [ '.$onetask->id.' | '.userdate($onesolution->timemodified).' ]</a><br>';
 								//echo 'onecategory='.$onecategory->id.' onetask='.$onetask->id.' onesolutionGRADE='.$onesolution->grade.'<br>';
								//echo $categoryTask[$size]->item[$onetask->id]->link;
							}
						}
					}
				}
        ...
        /*    
            ...
            // set the links to student information based on multiview or individual... if individual go to student info... if many go to individual grades view.
                if (has_capability('moodle/course:viewcoursegrades', $context)) {
                    if ($view_by_student != -1) {
                        $studentviewlink = '<a href="'.$CFG->wwwroot.'/user/view.php?id='.$student.'&amp;course='.$course->id.'">';
                    }
                    else {
                        $studentviewlink = '<a href="?id='.$course->id.'&amp;action=vcats&amp;user='.$student.'&amp;cview='.$cview.'">';
                    }
                    $studentviewlink .= $grades_by_student[$student]['student_data']['firstname'].' '.$grades_by_student[$student]['student_data']['lastname'].'</a>';
                    //$row .= '<th class="fullname" scope="row">'.$student_link.'</th>';
                     $studentRow[$mrowcount]->studviewlink = $studentviewlink;
                }
                            ...
                            if ($assignment != 'stats') {
                                ...
                                if ($first == 0) {
                                    ...
                                    if ($all_categories[$category][$assignment]['hidden'] == 0) {
                                        //$headerviewlink .= '<a href="'.$link.'">'.format_string($assignment,true).'</a>';
										$headerviewlink .= format_string($assignment,true);
                                    }
                                    else {
                                        //$headerviewlink .= '<a class="dimmed" href="'.$link.'">'.format_string($assignment,true).'</a>';
										$headerviewlink .= format_string($assignment,true);
                                    }
                                    if ($all_categories[$category][$assignment]['extra_credit'] == 1) {
                                        $headerviewlink .= '('.get_string('extracredit','grades').')'; 
                                    }
                                    $headerArray[sizeof($headerArray)]->headerviewlink = $headerviewlink;
                                    //$header .='</th>';
                                    if ($preferences->show_points) {
                                        $header1 .= $all_categories[$category][$assignment]['maxgrade'];
                                    ...
                                    if($preferences->show_percent)    {
                                        if ($all_categories[$category][$assignment]['grade_against'] != $all_categories[$category][$assignment]['maxgrade']) {
                                            $header1 .= get_string('scaledpct','grades');
                                        }
                                        else {
                                            $header1 .= get_string('rawpct','grades');
                                        }
                                    }
                                    if ($preferences->show_weighted) {
                                        ...
                                        $header1 .= $cur_weighted_max.get_string('pctoftotalgrade','grades');
                                    }
                                }
                                ...
                                if ($preferences->show_points) { 
                                    ...
                                    $oddcol = !$oddcol;
                                    $studentRow[$mrowcount]->item[$mcolcount] .= '<a href="'.$all_categories[$category][$assignment]['link'].'">' . $items[$assignment]['grade'] . '</a>';
                                ...
                                if ($preferences->show_percent) {
                                    ...
                                    $oddcol = !$oddcol;
                                    $row .= $items[$assignment]['percent'].'%';
                                }
                                
                                if ($preferences->show_weighted) {
                                    $class .= ($oddcol) ? ' c0 weighted' : ' c1 weighted';
                                    $oddcol = !$oddcol;
                                    $row .= $items[$assignment]['weighted'].'%';
                                }
                            }
                        }
                    } else {
                ...
                if ($first == 0) {
                    ...
                    if ($all_categories[$cview]['stats']['drop'] != 0) {
                        $header .= 'Additional total/grade ='.get_string('total','grades').'&nbsp; (Lowest '. $all_categories[$cview]['stats']['drop']. ' Dropped)'.$total_sort_link.' '.$stats_link;
                    }
                    else {
                        $header .= get_string('total','grades').'&nbsp;'.$total_sort_link.' '.$stats_link;
                    }
                        $headerArray[sizeof($headerArray)]->headerviewlink = get_string('total','grades');
                     
                    if ($preferences->show_points) {
                        $header1 .= $all_categories[$cview]['stats']['totalpoints'];
                    ...
                    if ($preferences->show_percent) {
                        $header1 .= get_string('percent','grades');
                    }
                    
                    
                    if ($preferences->show_weighted) {
                        $header1 .= $all_categories[$cview]['stats']['weight'].get_string('pctoftotalgrade','grades');
                    }
                ...
                if ($preferences->show_points) {
                    $mcolcount=sizeof($studentRow[$mrowcount]->item);
                    $studentRow[$mrowcount]->item[$mcolcount] .= $grades_by_student[$student][$cview]['stats']['points'];
                    //$class .= ($oddcol) ? ' c0 points' : ' c1 points';
                    //$oddcol = !$oddcol;
                ...
                if ($preferences->show_percent) {
                    $class .= ($oddcol) ? ' c0 percent' : ' c1 percent';
                    $oddcol = !$oddcol;
                    $row .= $grades_by_student[$student][$cview]['stats']['percent'].'%';
                }
                ...
                if ($preferences->show_weighted) {
                    $class .= ($oddcol) ? ' c0 weighted' : ' c1 weighted';
                    $oddcol = !$oddcol;
                    $row .= $grades_by_student[$student][$cview]['stats']['weighted'].'%';
                }
...
if ($csol)
{
            ...
            //foreach ($headerArray as $oneheadercat)
            //{
            //    echo "data2[data2.length] = new Array('".$oneheadercat->headerviewlink."'";
            //    echo ",'";
            //    foreach ($categoryTask as $onecategory)
            //    {
            //        foreach ($onecategory->item as $onetask)
            ...
            for ($i=1;$i<$length;$i++)
            {
                ...
                '<a href="'.$CFG->wwwroot.'/mod/vinttaskbook/view.php?id=35&cid='.$headerArray[$i]->id.'">'.$headerArray[$i]->headerviewlink.'</a>'."'";
                echo ",'";
                $tt=sizeof($categoryTask[$i]->item);
                $count='0';
            ...
            //for ($i=1;$i<$length;$i++)
            //{
            //    echo "data2[data2.length] = new Array('".$headerArray[$i]->headerviewlink."'";
            //    echo ",'";
            //    foreach ($categoryTask[$i]->items as $onetask)
            //    {
            //        echo $onetask."+";
            //    }
            //    echo " ',";
            ...
            //if ($SrCatCount!='0')
            //{
            //    echo "'".number_format($SrCat/$SrCatCount, 2, ',', ' ')."'";
            //} else
            //{
            //    echo "'".number_format($SrCat, 2, ',', ' ')."'";
            //}
			if ($totalCount!='0')
            {
				//echo "'".number_format($totalSum, 2, ',', ' ')."|13'";
...
else
{
            ...
            for ($i=1;$i<$length;$i++)
            {
                ...
                //echo "{text:'".$headerArray[$i]->headerviewlink."',align:'left',title:'title',sort:'alpha',width:'10%'}";
            }
            echo ',';
			...
			for ($i=0;$i<$studlength;$i++)
            {
                ...
                '<a href="'.$CFG->wwwroot.'/mod/vinttaskbook/view.php?id=35&cid='.$studArray[$i]->id.'">'.$studArray[$i]->name.'</a>'."'";
                echo ",";
                $tasklength=sizeof($categoryTask[$i]->item);
                $count='0';
                
				//$ttlength = sizeof($categoryTask[$i]->item);
				for ($j=0;$j<$allquizcount-1;$j++)
				{
                        ...
                        //{
                            if ($categoryTask[$i]->item[$j]->grade > '0')
							{
								...
								//echo $categoryTask[$i]->item[$j]->grade;
							} else
							{
								echo "'0',";
							}
...
function get_all_students($view_by_student)
{
	if ($view_by_student != -1) 
	{
		$filter = 'id='.$view_by_student;
		$students = get_records_select('user',$filter, 'firstname', 'id, firstname, lastname');
	}
	else
	{
		$students = get_records_select('user','', 'firstname', 'id, firstname, lastname');
	}
	return $students;
}
...
{ 
    $solutions = get_records_select('vinttaskbook_solutions', '', 'userid', 'id, userid, grade');
	return $solutions;
}
...
{ 
    $searchsql = "
	    SELECT Count(*)
	    FROM `mdl_vinttaskbook_categories`
	";
	return count_records_sql($searchsql);
}

function all_cur_quiz_count()
{ 
    $searchsql = "
	    SELECT Count(*)
	    FROM `mdl_quiz`
...
function get_one_student_link($onestudent)
{
	//if ($viewbystudent != -1) {
	//	$link = '<a href="'.$CFG->wwwroot.'/user/view.php?id='.$onestudent->id.'&amp;course='.$course->id.'">'.
	//			$onestudent->firstname.' '.$onestudent->lastname.'</a>';
	//}
    //else {
    //    $link = '<a href="?id='.$course->id.'&amp;action=view_student_grades&amp;user='.$onestudent->id.'">'.
	//			$onestudent->firstname.' '.$onestudent->lastname.'</a>';
    //}
	$link = $onestudent->firstname.' '.$onestudent->lastname;
	return $link;
}

function get_quiz_grades()
{
	$quiz_grades = get_records_select('quiz_grades', '', 'id', 'id, quiz, userid, grade');
	return $quiz_grades;
}

function get_all_cur_quiz()
{
	$cur_quiz = get_records_select('quiz', 'id != 3', 'id', 'id, name');
	return $cur_quiz;
}
...
// displays all grades for the course
	...
	//foreach ($allstudents as $onestudent)
	//{
	//	echo "studid=".$onestudent->id."firstname=".$onestudent->firstname." lastname=".$onestudent->lastname;
	//}


//all chapters from vinttaskbook 
//$select = "course = $course->id";
//$ch = get_records_select('vinttaskbook', $select, 'id', 'id, name');

//foreach($ch as $onechapter) {
//            echo "chapters:";
//            echo 'id='.$onechapter->id.' name='.$onechapter->name;
//}
...
//all solutions from vinttaskbook
$vinttaskSolutionGrade = get_all_solutions();
...
//$Sum='0';
//$Num='0';
//foreach($vinttaskSolutionGrade as $oneSolutionGrade) {
//	if ($oneSolutionGrade->userid == '85') 
//	{
//		$Num++;	
//		$Sum+=$oneSolutionGrade->grade;		
//	}
//}
//if ($Num != '0')
//{
//	echo 'Sum='.$Sum.' Num='.$Num.' Sr='.number_format($Sum/$Num, 2, ',', ' ');
//}
...
//$vinttaskcategories = get_records("vinttaskbook_categories", "vinttaskbook", $vinttaskbook->id);
//$select = "vinttaskbook = $vinttaskbook->id";
//$vinttasktasks = get_records_select('vinttaskbook_solutions', $select, 'vinttasktask', 'id, vinttasktask, userid, grade, language');
...
//echo 'userid='.$USER->id;
   ...
   // output a form for the user to download the grades.  
      ...
      //  $reprint=0;
        ...
        //fill student header title
        //$headerArray[sizeof($headerArray)]->headerviewlink = $header;
		$headerArray[sizeof($headerArray)]->headerviewlink = get_string('student','grades');
        //fill solution header title
        //$headerArray[sizeof($headerArray)]->headerviewlink = $vinttaskbook->name;
		$headerArray[sizeof($headerArray)]->headerviewlink = get_string('quiz_task','grades');
        //fill quiz_curr header title
		$headerArray[sizeof($headerArray)]->headerviewlink = get_string('quiz_curr','grades');
		//fill quiz_final header title
		$headerArray[sizeof($headerArray)]->headerviewlink = get_string('quiz_fin','grades');
		//fill total header title 
		$headerArray[sizeof($headerArray)]->headerviewlink = get_string('total','grades');
        
        //$studentRow[sizeof($studentRow)]->student = 'h0';
        //$studentRow[sizeof($studentRow)]->student = 'h1';
        //echo 'sizeofarray'.sizeof($studentRow).'hhh='.$studentRow[1]->student;
        
        
        //$studentColumn[1]->header = 'h1';
        //$studentColumn[2]->header = 'h2';
        //$studentColumn[0]->result = '0';
        //$studentColumn[1]->result = '1';
        //$studentColumn[2]->result = '2';
        //echo $studentColumn[2]->result;
        //for ($i=0;$i<3;$i++)
        //{
        //    echo $studentColumn[$i]->header;
        //}
        $mrowcount = 0;
        $mcolcount = 0;
        $studentRow=null;
            ...
            //get categories for current vinttaskbook-id from vinttaskbook_categories
            //$select = "vinttaskbook =". $vinttaskbook->id; 
            //$vinttaskcategories = get_records_select('vinttaskbook_categories', $select, 'number', 'id, number, name');
            //$vinttaskcategories = get_records("vinttaskbook_categories", "vinttaskbook", $vinttaskbook->id, "number");
            
            //get tasks for onecategory-id from vinttaskbook_tasks
            //$select = "vinttaskbook =". $vinttaskbook->id; 
            //$vinttasktasks = get_records_select('vinttaskbook_tasks', $select, 'number', 'id, number, name, vinttaskcategories');
            //$vinttasktasks = get_records("vinttaskbook_tasks", "vinttaskcategories", $vinttaskbook->id, "number");         
            
            //all solutions from vinttaskbook
            //$select = "vinttaskbook = 1";//hardcode greatings to vinttaskbook authors :( 
            //$vinttaskSolutionGrade = get_records_select('vinttaskbook_solutions ', $select, 'userid', 'id, userid, vinttasktask, grade');



		foreach($allstudents as $onestudent ) {
	    //$grades_by_student=$allstydents;
        //foreach($grades_by_student as $student => $categories) {
          ...
          //  $total_bonus_points = 0;
            ...
            // set the links to student information based on multiview or individual... if individual go to student info... if many go to individual grades view.
            //if (has_capability('moodle/course:viewcoursegrades', $context)) {
			//$studentviewlink = get_one_student_link($onestudent, $view_by_student);
            $studentRow[$mrowcount]->studviewlink = get_one_student_link($onestudent);
			$studentRow[$mrowcount]->studviewlink = mb_eregi_replace("'","\'", $studentRow[$mrowcount]->studviewlink);
			//echo $studentRow[$mrowcount]->studviewlink;
            //}
            //ksort($categories);

            //fill solution items
			//echo '<br>studid='.$student;
			$taskSum='0';
			$taskNum='0';
			$taskSr='0';
            $sollink='';
			foreach($vinttaskSolutionGrade as $oneSolutionGrade) {
				...
				if ($oneSolutionGrade->userid == $onestudent->id) 
				{
					if ($oneSolutionGrade->grade!='0')
					{
						$taskNum ++;	
						$taskSum +=$oneSolutionGrade->grade;
					}
				}
			}
			//$categoriesCount = all_categories_count();
			if ($taskNum != '0')
			{
				//$taskSr = $taskSum/($categoriesCount*$taskNum);
				$taskSr = $taskSum/$taskNum;
			}
			else
			{
				$taskSr = '0';
			...
			//echo '<br>taskSum='.$taskSum.' taskNum='.$taskNum.' taskSr='.$taskSr;
			...
			foreach ($vinttaskcategories as $onecategory)
		    {
				...
				{	
					if ($onetask->vinttaskcategories==$onecategory->id)
					{
						...
						foreach ($vinttaskSolutionGrade as $onesolution)
						{
							if (($onesolution->userid==$onestudent->id)&&($onetask->id == $onesolution->vinttasktask)&&($onesolution->grade !='0'))
							{
								$taskSum += $onesolution->grade;
								
								//$categoryTask[$onecategory->id]->item[$onetask->id]->grade = $onesolution->grade;
								//$categoryTask[$onecategory->id]->item[$onetask->id]->link = '<a href="'.$CFG->wwwroot.'/mod/vinttaskbook/edit.php?id=35&amp;cid=';
								//$categoryTask[$onecategory->id]->item[$onetask->id]->link .= $onecategory->id.'&amp;tid='.$onetask->id.'&amp;solid=';
								//$categoryTask[$onecategory->id]->item[$onetask->id]->link .= $onesolution->id.'&amp;type=solution&amp;action=view">';
								//$categoryTask[$onecategory->id]->item[$onetask->id]->link .= $onesolution->grade.'</a>';
 								echo 'onecategory='.$onecategory->id.' onetask='.$onetask->id.' onesolutionGRADE='.$onesolution->grade.'<br>';
							}
						}
					}
...
*/
			...
			$sollink .= '<a href="?id='.$course->id;
			$sollink .= '&amp;action=vcats&amp;user='.$onestudent->id.'&amp;csol=true&amp;cview=  ">';
			$sollink .= number_format($taskSr, 2, ',', ' ') .'</a>';
			$studentRow[$mrowcount]->item[$mcolcount] = $sollink;
			...
			foreach ($quizGrades as $oneQuizGrades)
			{
				//echo "userid=".$oneQuizGrades->userid." grade=".$oneQuizGrades->grade."<br>";
				if ($oneQuizGrades->userid==$onestudent->id)
				{
					if ($oneQuizGrades->id=='3')
					{
						$quiz_fin = $oneQuizGrades->grade/2;
					} else
					{
						$quiz_curr_Num ++;
						$quiz_curr_Sum += $oneQuizGrades->grade/2;
			...
			//print curr_Sr quiz Result
			$mcolcount=sizeof($studentRow[$mrowcount]->item);
			$studentRow[$mrowcount]->item[$mcolcount] = number_format($quiz_curr_Sr, 2, ',', ' ');
			//start
			//$sollink .= '<a href="?id='.$course->id;
			//$sollink .= '&amp;action=vcats&amp;user='.$onestudent->id.'&amp;csol=true&amp;cview='.$vinttaskbook->name.'">';
			//$sollink .= number_format($taskSr, 2, ',', ' ') .'</a>';
			//$studentRow[$mrowcount]->item[$mcolcount] = $sollink;
			...
			//print quiz fin Result
			$mcolcount=sizeof($studentRow[$mrowcount]->item);
			$studentRow[$mrowcount]->item[$mcolcount] = number_format($quiz_fin, 2, ',', ' ');
			...
			$totalNum ='3';
			$totalSum = $quiz_curr_Sr+$quiz_fin; //(string)(float)($studentRow[$mrowcount]->item[1]+$studentRow[$mrowcount]->item[2]);
			//for ($i=1;i<=$mcolcount;$i++)
			//{
			//	$totalNum++;
			//	$totalSum += $studentRow[$mrowcount]->item[$i];				
			//}
			//echo "totalsum".$totalSum;
			if ($totalNum != '0')
			{
				$totalSr = ($totalSum+$taskSr)/($totalNum);
				//echo $onestudent->firstname."<br>";
				//echo "totalSum= ".$totalSum."taskSum=". $taskSr."<br>"; 
				//echo "totalSr= ".$totalSr."<br>";
			}
			else
			{
				$totalSr = '0';
			}
			//print total_Sr Result
			$mcolcount=sizeof($studentRow[$mrowcount]->item);
			$studentRow[$mrowcount]->item[$mcolcount] =  number_format($totalSr, 2, ',', ' ');
                ...
                if ($category != 'student_data') {
                    ...
                    if ($first == 0) {
                        ...
                        if ($preferences->show_weighted || $preferences->show_percent || $preferences->show_points) {
                            ...
                            $headerviewlink.= '<a href="?id='.$course->id.'&amp;action=vcats&amp;cview='.$category;
                            
                            if ($view_by_student != -1) {
                                $headerviewlink .= '&amp;user='.$view_by_student;
                            }
                            $headerviewlink .='">'. $categoryname .'</a>';
                            //echo 'headerviewlink='.$headerviewlink.'category='.$category;
                            $headerArray[sizeof($headerArray)]->headerviewlink = $headerviewlink;
                            //echo 'step1'.$header.'category='.$category;
                            //echo sizeof($headerArray).$headerArray[sizeof($headerArray)]->headerviewlink;
                        }
                        ...
                        //$header .= '</th>';
                        if ($preferences->show_points) {
                            $header1 .= get_string('points','grades').'('.$all_categories[$category]['stats']['totalpoints'].')';
                            if ($all_categories[$category]['stats']['bonus_points'] != 0) {
                        ...
                        if ($preferences->show_weighted) {
                            $header1 .= get_string('weightedpctcontribution','grades');
                        }
                    ...
                    $mcolcount=sizeof($studentRow[$mrowcount]->item);
                    if ($preferences->show_points) {
                        $studentRow[$mrowcount]->item[$mcolcount] = number_format($items['stats']['points'], 2, ',', ' ');
                        //$row .= 'show_points=' . $items['stats']['points'];
                    }
                    if ($preferences->show_percent) {
                        $row .= 'show_percent='. $items['stats']['percent'].'%';
                    }

                    if ($preferences->show_weighted) {
                        $row .= 'show_weighted='. $items['stats']['weighted'] . '%';
                    }
            ...
            if ($first == 0) {
                ...
                $header .= 'Additional total/grade ='.get_string('total','grades').'&nbsp;'.$stats_link;
                $headerArray[sizeof($headerArray)]->headerviewlink = get_string('total','grades');
                if (has_capability('moodle/course:viewcoursegrades', $context) && $view_by_student == -1) {
                    if ($preferences->show_points) {
                        $header1 .= 'start header1 = '.get_string('points','grades').'('.$all_categories['stats']['totalpoints'].')';
                        if ($category != 'student_data' && $all_categories[$category]['stats']['bonus_points'] != 0) {

                            $header1 .='(+'.$total_bonus_points.')';
                        }
                        $header1 .= '<br />'.$points_sort_link.' ';
                    }
                    if ($preferences->show_percent) {
                        $header1 .= 'percentshort='.get_string('percentshort','grades').'<br />'.$percent_sort_link.' ';
                    }
                    if ($preferences->show_weighted) {
                        $header1 .= 'weightedpct='.get_string('weightedpct','grades').'('.$all_categories['stats']['weight'].')'.'<br />'.$weighted_sort_link.' ';
                    }
                    if ($preferences->show_letters) {
                        $header1 .= 'lettergrade='.get_string('lettergrade','grades').'<br />'.$grade_sort_link.' ';
                    }
                    //$header1 .= '</tr>';
                }
                else {
                    if ($preferences->show_points) {
                        $header1 .= 'secondary points/grade ='.get_string('points','grades').'('.$all_categories['stats']['totalpoints'].')';
                        if ($category != 'student_data' && $all_categories[$category]['stats']['bonus_points'] != 0) {
                            $header1 .='(+'.$total_bonus_points.')';
                        }
                        //$header1 .= '</th>';
                    ...
                    if ($preferences->show_percent) {
                        $header1 .= 'percentshort='.get_string('percentshort','grades');
                    }
                    if ($preferences->show_weighted) {
                        $header1 .= 'weightedpct='.get_string('weightedpct','grades').'('.$all_categories['stats']['weight'].')';
                    }
                    if ($preferences->show_letters) {
                        $header1 .= 'lettergrade='.get_string('lettergrade','grades');
                    }
                    //$header1 .= '</tr>';
                }
                //if (has_capability('moodle/course:viewcoursegrades', $context)) {
            ...
            if ($preferences->show_points) {
                $mcolcount=sizeof($studentRow[$mrowcount]->item);
                $studentRow[$mrowcount]->item[$mcolcount] = number_format($grades_by_student[$student]['student_data']['points']+$Sr, 2, ',', ' ');
                //echo 'mr='.$mrowcount.', mc='.$mcolcount.' need='.$grades_by_student[$student]['student_data']['points'].' get='.$studentRow[$mrowcount]->item[$mcolcount];
                //$row .= 'grades_by_student_point='.$grades_by_student[$student]['student_data']['points'];
            }
            if ($preferences->show_percent) {
                $row .= 'grades_by_student_percent='.$grades_by_student[$student]['student_data']['percent'].'%';
            }
            if ($preferences->show_weighted) {
                $row .= 'grades_by_student_weighted'.$grades_by_student[$student]['student_data']['weighted'].'%';
            }
            if ($preferences->show_letters) {
                if ($preferences->use_weighted_for_letter == 1) {
                ...
                if ($letter_grade) {
                    $row .= 'letter_grade='.$letter_grade->letter;
                }
                else {
                    // there wasn't an appropriate entry to use in the gradebook.
                    if (grade_letters_set($course->id)) {
                        $row .= 'nolettergrade='.get_string('nolettergrade','grades').' '.$grade;
                    }
                    else {
                        $row .= 'nogradeletters='.get_string('nogradeletters','grades');
                    }
                }
            }
            //if (has_capability('moodle/course:viewcoursegrades', $context)) {
...
//            }
        ...
        */
            ...
            //foreach ($headerArray as $oneHeader)
            //{
            //    echo $oneHeader->headerviewlink.'<br>';    
            //}
            
            //foreach ($studentRow as $oneStudentRow)
            //{
            //    echo /*sizeof($studentRow).*/$oneStudentRow->studviewlink.'<br>';
            //    //echo 'hhhhh='.$oneStudentRow->item[0];
            //    foreach ($oneStudentRow->item as $oneItem)
            //    {
            //           echo '---- '.$oneItem.', <br>'; 
            //    }
            //}
            ...
            echo "{text:'".$headerArray[0]->headerviewlink."',align:'left',title:'".$headerArray[0]->headerviewlink."',sort:'alpha',width:'10%'}";
            for ($i=1;$i<$length;$i++)
            {
                echo ',';
                echo "{text:'".$headerArray[$i]->headerviewlink."',align:'left',title:'".$headerArray[$i]->headerviewlink."',sort:'numeric', width:'10%'}";     
            }
            ...
            {
				$ttemp='';
                        //echo $oneStudentRow->studviewlink.'<br>';
                        //echo $oneStudentRow->studviewlink;
                $ttemp = 'data[data.length] = new Array(\''.format_string($oneStudentRow->studviewlink,true).'\'';
				//$ttemp = "data[data.length] = new Array('TempStudent'";
                foreach ($oneStudentRow->item as $oneItem)
                {
                    //$ttemp .= ",";        
                    $ttemp .= ',\''.$oneItem.'\'';
