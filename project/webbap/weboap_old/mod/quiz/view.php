<?php  // $Id: view.php,v 1.109.2.5 2007/08/06 13:22:08 tjhunt Exp $
        ...
        if ($attempts) {
            ...
            foreach ($attempts as $attempt) {
                ...
                if ($feedbackcolumn) {
					...
					$text = quiz_feedback_for_grade($attemptgrade, $quiz->id); // текст
					$pieces = explode(" ", $text);
        ...
        if ($numattempts && $quiz->sumgrades && !is_null($mygrade)) {
            if ($overallstats) {
                if ($available && $moreattempts) {
					...
					print_heading(get_string('yourfinalgradeis', 'quiz', "$text"));
                }
            }
